...
exports.Run = async function Run(caller, command, GUILD) {
  ...
  if (command.msg.author.id === process.env.OWNER || command.msg.member.permission.has('manageGuild') || command.msg.member.roles.indexOf(guild.suggestionRole) !== -1) {
    if (!command.params[0]) {
      caller.utils.message(command.msg.channel.id, {
        embed: {
          ...
          title: lang.title,
          description: `**${command.prefix}${lang.deny.help}\n\n${lang.example}${command.prefix}deny ${command.msg.id} Maybe in the future.\n${command.prefix}deny ${Math.random().toString(36).substr(2, 5)} We aren't looking for something like this right now.`,
        },
      });
      return;
    }
    ...
    try {
      ...
      if (e.code === 50013) {
        caller.utils.message(command.msg.channel.id, {
          embed: {
            title: lang.titleError,
            description: lang.deny.read[0] + guild.suggestion + lang.deny.read[1],
            color: caller.color.yellow,
          },
        });
        guild.suggestion = '';
    ...
    if (suggestion && guild.suggestionDM) {
      ...
      caller.utils.message(channel.id, lang.deny.dm.replace('$USER', command.msg.author.username).replace('$SUGGESTION', suggestion.suggestion).replace('$REASON', reason));
    }
  } else {
    caller.utils.message(command.msg.channel.id, {
      embed: {
